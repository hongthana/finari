...
export async function sendMagicLinkEmail(params: {
  ...
  const requestContext = activityRequestContext({
    headers: requestHeaders,
    method: "POST",
    url: "https://www.finari.co/api/auth/signin/email",
  });
  const email = normalizeEmail(params.identifier) ?? params.identifier.trim().toLowerCase();
  const emailHash = hashActivityValue(email);

  await recordActivityEvent({
    emailHash,
    category: "auth",
    eventName: "auth.magic_link.requested",
  ...
  if (!rateLimit.allowed) {
    await recordActivityEvent({
      emailHash,
      category: "auth",
      eventName: "auth.magic_link.throttled",
      path: requestContext.path,
      method: requestContext.method,
      status: "blocked",
      ipHash: requestContext.ipHash,
    ...
    throw new Error("Too many sign-in links requested. Try again later.");
  }
  ...
  if (!apiKey) {
    if (process.env.NODE_ENV !== "production") {
      ...
      await recordActivityEvent({
        emailHash,
        category: "auth",
        eventName: "auth.magic_link.sent",
        path: requestContext.path,
        method: requestContext.method,
        status: "ok",
        ipHash: requestContext.ipHash,
    ...
    await recordActivityEvent({
      emailHash,
      category: "auth",
      eventName: "auth.magic_link.failed",
      path: requestContext.path,
      method: requestContext.method,
      status: "error",
      ipHash: requestContext.ipHash,
      userAgentHash: requestContext.userAgentHash,
      metadata: { provider: "resend", reason: "missing_api_key" },
    });
