...
```bash
pnpm backfill:sp500 -- --base-url https://your-finari-domain.example --limit 5 --delay-ms 2500
```

Run the full S&P 500 backfill conservatively:

```bash
pnpm backfill:sp500 -- --base-url https://your-finari-domain.example --delay-ms 3000 --max-failures 25
```

Resume after the last successful ticker from the JSONL report:

```bash
pnpm backfill:sp500 -- --base-url https://your-finari-domain.example --start-after MSFT --delay-ms 3000
```
